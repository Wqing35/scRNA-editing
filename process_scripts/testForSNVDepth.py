...
def main(): 
    ...
    output_directory='/disk1/wenqing/tmp_data/PFC_s2/test_filter_in_snv_neuron/'
    #根据supporting reads对snv结果进行过滤（5-20）
    filter_idx = range(1, 51)
    for idx in filter_idx:
        fi_regular_snv=open('./regular.snv')
        fo_regular_snv=open(output_directory+'regular.snv.filtered.'+str(idx),'w')
        ...
        output_directory_sub=output_directory+'/filter_idx'+str(idx)+'/'
        ...
        annotate(output_directory+'regular.snv.filtered.'+str(idx),repeat,output_directory_sub+'regular.snv.anno')    
        #根据注释将snv分成3个部分
        ...
        get_depth( output_directory+'/all_combined.zz.sorted' , output_directory_sub+'/regular.res', output_directory_sub+'/regular.res.depth')
