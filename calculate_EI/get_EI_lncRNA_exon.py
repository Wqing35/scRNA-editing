...
def main():
    ...
    def get_AEI(element,region,depth_index):
        ...
        if os.path.exists('./'+element+'_combined.zz.sorted.modified_lncRNA_exon_trimmed_reads_OnlyAG')==False:
            ...
            print 'Get reads in lncRNA exon:'

            #筛选位于gene区域的reads
            subprocess.Popen('bash '+'/disk1/wenqing/scRNA-editing/calculate_EI/get_reads_lncRNA_Exon'+'.sh',shell=True).wait()
            ...
            print 'Trim reads according to lncRNA exon:'

            fi=open(element+'_combined.zz.sorted.modified_lncRNA_exon')
            fo=open(element+'_combined.zz.sorted.modified_lncRNA_exon_trimmed_reads_OnlyAG','w')
        ...
        EI_out_dir=element+'_EI_lncRNA_exon_over'+str(depth_index)+'.txt'
        gene_counts_in_dir=element+'_combined.zz.sorted.modified_lncRNA_exon_trimmed_reads_OnlyAG'
    ...
    #for idx in dp_idx:
    idx=0
    get_AEI('regular','lncRNA_exon',idx)
    print 'done'
