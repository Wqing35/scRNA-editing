...
def main():
    ...
    def get_AEI(element,region,depth_index):
        ...
        if os.path.exists('./'+element+'_combined.zz.sorted.modified_mRNA_exon_trimmed_reads_OnlyAG')==False:
            ...
            print 'Get reads in mRNA exon:'

            #筛选位于gene区域的reads
            subprocess.Popen('bash '+'/disk1/wenqing/scRNA-editing/calculate_EI/get_reads_mRNA_Exon'+'.sh',shell=True).wait()
            ...
            print 'Trim reads according to mRNA exon:'

            fi=open(element+'_combined.zz.sorted.modified_mRNA_exon')
            fo=open(element+'_combined.zz.sorted.modified_mRNA_exon_trimmed_reads_OnlyAG','w')
        ...
        EI_out_dir=element+'_EI_mRNA_exon_over'+str(depth_index)+'.txt'
        gene_counts_in_dir=element+'_combined.zz.sorted.modified_mRNA_exon_trimmed_reads_OnlyAG'
