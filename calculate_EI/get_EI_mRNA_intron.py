...
def main():
    ...
    def get_AEI(element,region,depth_index):
        ...
        if os.path.exists('./'+element+'_combined.zz.sorted.modified_mRNA_intron_trimmed_reads_OnlyAG')==False:
            ...
            print 'Get reads in mRNA intron:'

            #筛选位于gene区域的reads
            subprocess.Popen('bash '+'/disk1/wenqing/scRNA-editing/calculate_EI/get_reads_mRNA_Intron'+'.sh',shell=True).wait()
            ...
            print 'Trim reads according to mRNA intron:'

            fi=open(element+'_combined.zz.sorted.modified_mRNA_intron')
            fo=open(element+'_combined.zz.sorted.modified_mRNA_intron_trimmed_reads_OnlyAG','w')
        ...
        EI_out_dir=element+'_EI_mRNA_intron_over'+str(depth_index)+'.txt'
        gene_counts_in_dir=element+'_combined.zz.sorted.modified_mRNA_intron_trimmed_reads_OnlyAG'
    ...
    #for idx in dp_idx:
    idx=0
    get_AEI('regular','mRNA_intron',idx)
    print 'done'
