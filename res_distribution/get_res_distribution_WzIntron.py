...
def main():
    ...
    def genebody_percentile_intron(refbed, mRNA_len_cut = 100):
        '''
        ...
        for line in open(refbed,'r'):
            try:
                ...
                geneName = geneName.replace(';', '') 
                #strand    = fields[6]
                geneID = '_'.join([str(j) for j in (chrom, tx_start, tx_end, geneName)])

                intron_starts = list(map( int, fields[4].rstrip( ',\n' ).split( ',' ) ))
                intron_ends = list(map( int, fields[5].rstrip( ',\n' ).split( ',' ) ))
                transcript_count += 1
            except:
            ...
            if len(gene_all_base) < mRNA_len_cut:
                continue
            g_percentiles[geneID] = (chrom,mystat.percentile_list (gene_all_base))	#get 100 points from each gene's coordinates
    ...
    gene_percentiles = genebody_percentile_intron(refbed = '/disk1/wenqing/tmp_data/gencode_hg19_intron_in_transcript.uniq.txt', mRNA_len_cut = 100) 
    ...
    for geneID, (chrom, strand, percentiles) in gene_percentiles.items():  
        for percentile in percentiles:  
            edit_counts_per_percentile[percentile] = 0 """ 
    edit_counts_per_percentile={}
    ...
    for index, row in edit_sites_df.iterrows():  
        ...
        # 遍历每个基因区域的划分结果  
        for geneID, (gene_chrom, percentiles) in gene_percentiles.items():  
            if chrom == gene_chrom :  
    ...
    # 将编辑位点计数转换为列表，以用于绘图  
    percentiles = sorted(edit_counts_per_percentile.keys())
    ...
    # 将数据框写入CSV文件  
    df.to_csv('~/output_intron_s2.csv', index=False)
