...
def main(): 
    ...
    def help_doc():
        ...
        print "   Example:"
        print ""
        print "       sprint main -rp hg19_repeat.txt -c 6 -cr 16 -p 6 -1 r2.fq -r1 r1.fq -br ./barcode.txt hg19.fa output ./bwa-0.7.12/bwa ./samtools-1.2/samtools"
        print ""
            ...
            if basequa[0]!='*' and mpqua >= limitmpqua and mpqua < 200:
                ...
                if len(baseqlst)>0 and sum(baseqlst)/float(len(baseqlst)) >= limitbasequa: #and missnum <= mismatch_num(len(seq[7])):
                    for snv in truesnv:
                        ...
                        try:
                            #allsnv：以chr+mismatch类型+位点+barcode为index，其对应的该snv的数量为值
                            ...
                            if zz_in_dir==output+"/tmp/genome_all.zz.dedup":
                                add_line=seq[8].split('_')[2]
                            else:
                                read_name_1=seq[8].split("_|_")[0]
    ...
    def snv_cluster(bed_in_dir=0,bed_out_dir=0,cluster_distance=-1,cluster_size=-1):
                ...
                if seq[0]==tmpseq[0] and int(seq[2])-int(tmpseq[1])<=limitdistance and seq[3]==tmpseq[2]:
                    ...
                    if len(lst)>=limitnum:
                            ...
                            if int(end-begin) != 0:
                                ...
                                if density >= (cluster_size/cluster_distance):
                                    tmp_dict[i]=lst
                                    i=i+1
                    lst=[]
                    lst.append(line)
        ...
        for i in range(0,len(tmp_dict)): 
            ...
            if len(set(cluster_sites)) > limitnum:
                """ one_cluster_size=len(set(cluster_sites))
                list = dict(Counter(cluster_barcodes))
                #dup_barcode_num：重复出现的barcode个数
                dup_barcode_num=len({key:value for key,value in list.items() if value > 1})
                overlap_index=dup_barcode_num/float(one_cluster_size)
                if overlap_index >= overlap_criterion: """
    ...
    if 1==1: 
        ...
        subprocess.Popen('echo "#Chrom\tStart(0base)\tEnd(1base)\tType\tSupporting_reads\tStrand\tbarcode\tAD:DP\n" | cat - '+tmp +'/all.res.depth   > '+output+'/SPRINT_identified_all.res',shell=True).wait()
        print 'finished !'
